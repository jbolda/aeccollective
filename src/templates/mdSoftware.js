      ...
      <SimpleNav sitemetadata={this.props.data.site.siteMetadata} location={this.props.location}>
        ...
        <div className="container content">
          ...
          <div className="tags">
            ...
            <span className="tag is-medium" key={tag}>{tag}</span>
            )}
          </div>
        </div>
        </section>
        <section className="section">
          <div className="container content">
            <h3 className="userSupport">
             USER SUPPORT
            </h3>
          <table>
            <tr>
            <td>User forums:</td>
            <td><a href={frontmatter.userForums.link}>{frontmatter.userForums.text}</a> </td>
            </tr>
            <tr>
            <td>Technical Support:</td>
            <td><a href={frontmatter.userSupport.link}>{frontmatter.userSupport.text}</a> </td>
            </tr>
            <tr>
            <td>Tutorials:</td>
            <td><a href={frontmatter.tutorials.link}>{frontmatter.tutorials.text}</a> </td>
            </tr>
          </table>
          </div>
        </section>
  ...
  query markdownTemplateBySoftware($slug: String!) {
    markdownRemark(fields: { slug: { eq: $slug } }) {
      ...
      frontmatter {
        ...
        tags
        logo 
        userForums {text, link}
        userSupport {text, link}
        tutorials {text, link}
        studentPricing
        professionalPricing
        description
    ...
    file(relativePath: { eq: "assets/logos/*" }) {
      childImageSharp {
        fluid(maxWidth: 256) {
